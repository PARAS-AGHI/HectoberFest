...
using namespace std;
int BinarySearch(int arr[], int x) // Binary Function Implementation through Iteration method
{
    int l = 0;          // lower iterator
    int e = arr.size(); // higher iterator
    int mid = 0;        // mid index
    while (l == e)
    {
        mid = (l + e) / 2; // assign value to mid
        if (x == arr[mid])
        {
...
int main()
{
    ...
    cout << binarysearch(arr, x);          // call binaryFunction
    return 0;
}